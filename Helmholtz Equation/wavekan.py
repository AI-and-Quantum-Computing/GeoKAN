...
class KANLinear(nn.Module):
    def __init__(self, in_features, out_features, wavelet_type='sin'):
    ...
    def wavelet_transform(self, x):
        ...
        if self.wavelet_type == 'mexican_hat':
            ...
            gaussian = torch.exp(-0.5 * x_scaled ** 2)
            wavelet  = gaussian
            wavelet_weighted = wavelet * self.wavelet_weights.unsqueeze(0).expand_as(wavelet)
            wavelet_output = wavelet_weighted.sum(dim=2)
    ...
    def forward(self, x):
        ...
        return combined_output#self.bn(combined_output)
wavelet = input("give the type of the wavelet")
class KAN(nn.Module):
    def __init__(self, layers_hidden, wavelet_type=wavelet):
    ...
    def forward(self, x: torch.Tensor, update_grid=False):
        ...
        for layer in self.layers:
            ...
            x = layer(x)
        return x
class KAN_twovariable(nn.Module):
    def __init__(self, layers_hidden, wavelet_type=wavelet):
    ...
    def forward(self, x: torch.Tensor, t_1: torch.Tensor, update_grid=False):
        #ini = torch.sin(np.pi * x)  # Incorporate initial condition
        x = torch.cat((x, t_1), dim=1)
        for layer in self.layers:
            if update_grid:
